...
class StateMachine(object):
    def __init__(self):
        ...
        self.robot_base_frame_name = rospy.get_param(rospy.get_name() + '/robot_base_frame')

        print(self.robot_base_frame_name)
        #print(self.pick_srv_name)
        #print(self.place_srv_name)
        #print(self.marker_pose_topic_name)
        #print(self.cube_pose_str)
        print("###############################")
        # Subscribe to topics
        ...
        rospy.wait_for_service(self.pick_srv_name, timeout = 20)
        rospy.wait_for_service(self.place_srv_name, timeout = 20)
    ...
    def get_posestamped(self,pose_str):
        pose = [float(p) for p in pose_str.split(',')]
        pose_posestamped = PoseStamped()

        pose_posestamped.header.stamp = rospy.Time.now()
        pose_posestamped.header.frame_id = self.robot_base_frame_name

        pose_posestamped.pose.position.x = pose[0]
        pose_posestamped.pose.position.y = pose[1]
        pose_posestamped.pose.position.z = pose[2]

        pose_posestamped.pose.orientation.x = pose[3]
        pose_posestamped.pose.orientation.y = pose[4]
        pose_posestamped.pose.orientation.z = pose[5]
        pose_posestamped.pose.orientation.w = pose[6]
        
        return pose_posestamped
    ...
    def check_states(self):
        ...
        while not rospy.is_shutdown() and self.state != 5:
            ...
            if self.state == 2:
                ...
                self.cube_pose_posestamped = self.get_posestamped(self.cube_pose_str)
                self.cube_pose_pub.publish(self.cube_pose_posestamped)
                print('************ Cube pose published! ************')

                pick_srv = rospy.ServiceProxy(self.pick_srv_name, SetBool)
                reply = pick_srv()
                ...
                if reply.success == True:
                    ...
                    print('Picking didnt not happen')
                    self.state = 6

                rospy.sleep(4)

            # State 3: Moving the robot
            if self.state == 3:
                vel_msg = Twist()
                vel_msg.angular.z = -1
            ...
            if self.state == 4:
                ...
                place_srv = rospy.ServiceProxy(self.place_srv_name, SetBool)
                reply = place_srv()

                if reply.success == True:
                    self.state = 5
                    print('******** Placing Successful! *******')
                else:
                    print('Placing didnt not happen')
                    self.state = 6
